...
const HANDLER_VERSION = "uploadPerfumeImage-v3-private-first";
...
export async function handleUploadPerfumeImage(req, res) {
  ...
  try {
    res.setHeader("x-aroma-upload-handler", HANDLER_VERSION);
  } catch {
    // ignore
  }
  ...
  try {
    ...
    // O Blob Store pode estar configurado como PRIVATE ou PUBLIC dependendo do ambiente.
    // Para evitar o erro "Cannot use public access on a private store", tentamos PRIVATE primeiro.
    // Se o store for PUBLIC, o SDK exige `access: "public"`.
    let blob;
    let usedAccess = "private";
    try {
    ...
    return res.status(200).json({ url: blob.url, access: usedAccess, v: HANDLER_VERSION });
  } catch (err) {
