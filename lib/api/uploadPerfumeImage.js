...
export async function handleUploadPerfumeImage(req, res) {
  ...
  try {
    ...
    // - Store PRIVATE: "public" falha ("Cannot use public access on a private store")
    // - Store PUBLIC: "private" falha ("access must be \"public\"")
    // Então tentamos e fazemos fallback automaticamente.
    let blob;
    try {
      ...
      } else if (msg.includes('access must be "public"') || msg.includes('access must be \\"public\\"')) {
        blob = await tryPut("public");
      } else {
        // tentativa alternativa: pode ter sido enviado primeiro como public e falhado por outro motivo.
        // tenta "private" só como fallback final.
        try {
          blob = await tryPut("private");
        } catch {
          throw err;
