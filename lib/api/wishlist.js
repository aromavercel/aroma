...
async function getWishlistUser(req, res) {
  ...
  if (!phone) {
    res
      .status(400)
      .json({ error: "Lista de desejos disponível apenas para usuários com telefone cadastrado" });
    return null;
  }
...
async function addWishlistItem(req, res) {
  ...
  try {
    ...
    if (!wishlist) {
      const [created] =
        await sql`INSERT INTO wishlists (user_phone) VALUES (${wUser.userPhone}) RETURNING id`;
      wishlist = created;
    }
