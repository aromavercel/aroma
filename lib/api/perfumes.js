...
async function handleList(req, res) {
  ...
  /** Filtro por preço na lista: não usar alias `price_min` no WHERE (inválido no PG). */
  const priceFilterOnPerfume = hasPriceFilter
    ? sql`
        AND (${priceMin}::numeric IS NULL OR (
        ...
        AND (${priceMax}::numeric IS NULL OR (
          SELECT MIN(NULLIF((v->>'price_number')::numeric, 0))
          FROM jsonb_array_elements(COALESCE(p.variants, '[]'::jsonb)) AS v
          WHERE (v ? 'price_number') AND (v->>'price_number') ~ '^[0-9]+(\.[0-9]+)?$'
        ) <= ${priceMax})
      `
    : sql``;
  /** Na CTE de contagem, `price_min` é coluna de `base`. */
  const priceFilterOnBase = hasPriceFilter
    ? sql`
        AND (${priceMin}::numeric IS NULL OR price_min >= ${priceMin})
        AND (${priceMax}::numeric IS NULL OR price_min <= ${priceMax})
      `
    : sql``;
  ...
  const orderSql =
    sort === "title-asc"
      ...
      : sort === "title-desc"
        ...
        : sort === "price-asc"
          ? sql`ORDER BY price_min ASC NULLS LAST, p.title ASC`
          : sort === "price-desc"
            ? sql`ORDER BY price_min DESC NULLS LAST, p.title ASC`
            : sql`ORDER BY p.catalog_source, p.title`;
    ...
    if (compactOnly) {
      rows = isPaged
        ? await sql`
            ...
            ${priceFilterOnPerfume}
            ${orderSql}
            LIMIT ${listLimit} OFFSET ${effectiveOffset}
          `
        : await sql`
            SELECT p.id, p.external_url, p.title, p.description, p.catalog_source, p.image_2_url, p.ativo, p.esgotado,
                   b.name AS brand_name,
