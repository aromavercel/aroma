...
function canonicalPerfumeImageUrl(u) {
...
/** Alinha variantes ao salvar: image_url que não está na galeria oficial vira órfã na vitrine. */
function stripVariantImagesNotInGallery(variants, galleryUrls) {
...
async function handlePostCreate(req, res) {
  ...
  const images = rawImages
    ...
    .filter(Boolean);
  if (images.length > 0) {
    variants = stripVariantImagesNotInGallery(variants, images);
  }
  const active =
    body.active != null
